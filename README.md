...
| **RE Village — VR scope** | RE Engine | 🎮 The rifle's inaccuracy is now a measured number rather than a feeling: fired from the hip it throws shots up to fifteen degrees off, while aiming brings that to almost nothing. That turned out to be the game deliberately scattering each bullet at the moment it is fired, so there is now a switch that cancels the scattering — written, in place, and waiting on one short test. If it holds, the rifle becomes accurate without needing the aim button at all. | 09-20 | [re-village-scope-vr](https://github.com/TefMeister/re-village-scope-vr) |
...
| **Ashes 2063** (2018) | GZDoom | 🔧 The missing half is built: a long gun is now **drawn** along the line between both hands, with the right hand as the anchor and the left hand swinging the muzzle, so the off hand finally does something you can see. Engine rebuilt and deployed, maths covered by its own tests — waiting on a headset to say whether it looks right | 09-20 | [ashes-2063-weapons](https://github.com/TefMeister/ashes-2063-weapons) |

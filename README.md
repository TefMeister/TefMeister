...
| **RE Village — VR scope** | RE Engine | 🎮 Worked out why the scope shows things it should not — his own coat, the gun, the ground, nearby branches. The picture is made by a mirror, and a mirror does not show the view from where it hangs; it shows your own view, bounced. Today the maths behind that bounce got pinned down properly: the picture is thrown off by exactly double the angle between where he is looking and the way the mirror lies, which is why it can feel like the camera is up in the sky. Also settled for good that the picture can never be taken from the end of the barrel — a mirror angled to do that is one you stare straight into, so it would show what is behind him. A new offline check proves all of it without running the game, and there is a five-word change to try next time it is on. | 09-18 | [re-village-scope-vr](https://github.com/TefMeister/re-village-scope-vr) |

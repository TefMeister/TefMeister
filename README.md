...
| **RE Village — VR scope** | RE Engine | 🎮 The rifle shoots exactly where it points, from the hip, no aim button needed. Two small aiming faults are left, and both are now set up to explain themselves: a steady miss to the left when holding the rifle with both hands (the aim correction tilts with the gun), and a wrong distance reading that came from an invisible ray catching something right at the muzzle. The mod now has its own ray that ignores anything that close and writes down what it hit. One short headset test settles both. | 09-21 | [re-village-scope-vr](https://github.com/TefMeister/re-village-scope-vr) |

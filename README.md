...
| **RE Village — VR scope** | RE Engine | 🎮 Working, zeroed sniper scope. Another stretch with the game switched off, and the pick of it was a small thing: when you put the rifle away and take it out again, the scope showed the game’s own dark glass for a second first — because the mod waited a fixed second before putting our picture back, a delay nobody had ever timed. It now puts it back the instant it can, and writes down how long that really took. Also from the same day: the picture going under the ground when crouched turned out not to be cut off at all, the settings file no longer saves one-off experiments as permanent, the tuning switches are buttons inside the headset, and the upside-down picture was traced to one setting used twice over. All installed and riding along with the next start. | 09-18 | [re-village-scope-vr](https://github.com/TefMeister/re-village-scope-vr) |
...
| **The Darkness** (2007) | Starbreeze engine | 🏆 **Stereo pair working, and the world can now be held still between the two eyes** — the game's program file also turned out to be openable, so every future question is cheaper. Next: prove the freeze on screen, then show the pair side by side | 09-18 | [the-darkness-vr](https://github.com/TefMeister/the-darkness-vr) |
...
| **Ashes 2063** (2018) | GZDoom | 🔧 Revolver in the right hand, lantern in the left — sized and turned as the player wanted, and the lantern genuinely lights the dark. Now working on holding a rifle with both hands: the groundwork was already there, so what was missing was only that the shot followed the rear hand's tilt instead of the barrel. The sums for aiming down the line between your hands are written and checked; the build and the wearing wait for the headset PC | 09-18 | [ashes-2063-weapons](https://github.com/TefMeister/ashes-2063-weapons) |

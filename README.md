...
| **RE Village — VR scope** | RE Engine | 🔧 First time the home PC has run this one, and it found something nobody expected: with the headset off, the picture inside the scope is simply black. Five launches said the same, and so did yesterday's build, so it is not something that broke today — the most likely explanation is that the picture has only ever been made when a headset is actually plugged in, and every check until now was done wearing one. That is being read through now. Plenty did get settled on the way: hiding the player's body has a working trigger, the sniper's spread is not held on the gun at all so that idea needs a different route, one-off tuning no longer leaks into the saved settings, and the game now runs here at 160–200 frames a second. | 09-18 | [re-village-scope-vr](https://github.com/TefMeister/re-village-scope-vr) |

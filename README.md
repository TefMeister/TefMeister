...
| **RE Village — VR scope** | RE Engine | 🎮 Working, zeroed sniper scope. A long day with the game switched off and five things closed, the best of them by reading rather than building: the picture going under the ground when crouched is not being cut off at all — the point it is taken from has sunk below the floor, and one of the three position sliders was moving it at double speed. Also: the settings file no longer saves one-off experiments as permanent, the tuning switches are buttons inside the headset, the log survives a restart, and the upside-down picture was traced to one setting used twice over. All installed and riding along with the next start. | 09-18 | [re-village-scope-vr](https://github.com/TefMeister/re-village-scope-vr) |
...
| **The Darkness** (2007) | Starbreeze engine | 🏆 **First true left-eye / right-eye picture, measured** — and the design for two eyes is decided: each eye gets a real picture of its own. First stereo bug found too (two heads lit differently per eye). Next: freeze the world between the eyes, then show the pair side by side | 09-18 | [the-darkness-vr](https://github.com/TefMeister/the-darkness-vr) |

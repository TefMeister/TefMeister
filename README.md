...
| **RE Village — VR scope** | RE Engine | 🎮 The fix for the smearing scope picture is now **written and built**, not just worked out. Instead of drawing a wider picture and losing sharpness, the scope's frame is now aimed at the barrel, so the bit it crops out is always in the middle of it and can never run off the edge. It sits behind an off-by-default switch with a hand slider, so it can be swept on a flat screen with no headset on before anything is trusted. Two things are honestly unknown: whether it steers the right way round (there is a one-tick cure if not), and whether the game keeps drawing everything it should at a steep angle — both need the game running. Still waiting on a wearer: does the smear ever show in one eye only? | 09-19 | [re-village-scope-vr](https://github.com/TefMeister/re-village-scope-vr) |

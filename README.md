...
| **RE Village — VR scope** | RE Engine | 🔍 Tested the smear fix in the game and it did **not** work — the code writes its numbers perfectly and the scope picture ignores them completely. That is a useful answer, not a wasted evening: it means the place we were changing is not the place that decides what the scope shows, and it casts doubt on the theory the whole fix was built on. The wearer also reports the smear is in both eyes, where the theory predicted one. A deliberately unmissable test is built and waiting — one relaunch will say for certain whether that part of the picture is ours to control. Also measured: the 35-second wait before the scope appears is 0.06 seconds of work and 28 seconds of caution, so it can be made near-instant. | 09-19 | [re-village-scope-vr](https://github.com/TefMeister/re-village-scope-vr) |

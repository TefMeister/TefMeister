...
| **RE Village — VR scope** | RE Engine | 🎮 Found why the scope picture smears and shakes when the rifle is held well off to one side, and it has an exact angle. The picture the scope crops out of is drawn at the headset's own field of view, so once the barrel points more than about 39 degrees away from where you are looking, the bit it wants is simply not in the frame, and it stretches the edge instead. The obvious cheap fix turned out to be backwards — it would have made the frame narrower still. Better answer: aim the frame at the barrel rather than widening it, which costs nothing in sharpness and removes the fault at any angle. Also predicted, and never looked for: there should be a band where one eye smears and the other does not. | 09-19 | [re-village-scope-vr](https://github.com/TefMeister/re-village-scope-vr) |

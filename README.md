...
| **RE Village — VR scope** | RE Engine | 🎮 Working, zeroed sniper scope. A long day with the game switched off, and four things closed: the settings file was saving one-off experiments as permanent choices, the tuning switches became buttons reachable inside the headset, the game's log now survives a restart, the upside-down picture was traced to one setting used twice over, and the reading that should have shown the rifle shaking turned out to be smoothing it away. All installed and riding along with the next start. | 09-18 | [re-village-scope-vr](https://github.com/TefMeister/re-village-scope-vr) |

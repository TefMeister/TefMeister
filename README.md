...
| **RE Village — VR scope** | RE Engine | 🎮 Two-handed shots now land on the crosshair, confirmed in the headset. The distance correction that was never reaching the scope picture is fixed and seen working in the headset, and the scope was re-zeroed with it on; shooting near and far with that zero is the check still to come. The 1080p scope picture meant for smoother play did not switch on in that run, so that is being looked into. | 09-21 | [re-village-scope-vr](https://github.com/TefMeister/re-village-scope-vr) |

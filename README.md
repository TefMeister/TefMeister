...
| **RE Village — VR scope** | RE Engine | 🎮 Still on the scope picture, and today on the hands. Read how a well-regarded VR mod for another game places the hand that holds the front of a rifle, and found it uses one trick for two problems: it nudges the target the support hand reaches for a few centimetres downward, in real-world space, before the gun is put in the hands — so the hand stops hiding behind the other one and ends up sitting lower than where you are actually holding the controller, which is where a support hand really goes. Also worked out why our own attempt at this did nothing: the mod we build on throws our number away the moment the hand touches the gun, and a thirty-second idle timer can freeze the hands entirely. Both now have a free test to tell them apart. | 09-19 | [re-village-scope-vr](https://github.com/TefMeister/re-village-scope-vr) |
...
| **The Darkness** (2007) | Starbreeze engine | 🏆 **Stereo pair working, and the world can be held still between the two eyes.** The whole game and our port are now packed and verified for the faster test machine — every file checked byte for byte before it travelled. Next: prove the freeze on screen, then show the pair side by side | 09-19 | [the-darkness-vr](https://github.com/TefMeister/the-darkness-vr) |
| **Condemned 2: Bloodshot** (2008) | Xbox 360 static recompilation (ReXGlue) | ⭐ Running on PC from a self-built recompile — the official build needs a 2013+ CPU, ours does not. Now packed and verified for the faster machine, to find the frame rate the stereo work depends on. Three fixes written up for upstream | 09-19 | [condemned-2-vr](https://github.com/TefMeister/condemned-2-vr) |
...
| **Silent Hill 2** (2024 remake) | Unreal Engine 5 | 🔍 Just started, and the only Unreal game here — which means the usual months of reverse engineering may not be needed. Reading two existing VR profiles first; nothing run yet | 09-18 | [silent-hill-2-remake-vr](https://github.com/TefMeister/silent-hill-2-remake-vr) |
| **Ashes 2063** (2018) | GZDoom | 🎮 The hand-built shotgun is in the game and was worn for the first time: it sat pointing low, which turned out to be an exact fault in how it was packed for VR, now corrected. The lantern in the off hand can finally be put away, which also frees that hand for holding a long gun with both. Ready for a second headset try | 09-19 | [ashes-2063-weapons](https://github.com/TefMeister/ashes-2063-weapons) |

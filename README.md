...
| **RE Village — VR scope** | RE Engine | 🎮 The rifle now shoots exactly where it points, from the hip, with no aim button — confirmed in game. The game turned out to build every bullet with a deliberately wobbled direction, up to fifteen degrees off; the mod now swaps the clean aim back in at the moment the bullet is made. It took five wrong attempts and one crash to find the right moment, and the lesson went into the lanes plugin. Next is zeroing the scope properly, which was never really possible while every shot was being thrown off at random. | 09-21 | [re-village-scope-vr](https://github.com/TefMeister/re-village-scope-vr) |

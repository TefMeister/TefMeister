...
| **RE Village — VR scope** | RE Engine | 🎮 The fix for the smearing scope picture is written, built and now **installed and switched on** in the game. Instead of drawing a wider picture and losing sharpness, the scope aims its frame at the barrel, so the part it crops out always sits in the middle and can never run off the edge. It comes with a one-click undo that has been tested both ways. Two honest unknowns remain, and both need the game actually running: whether it steers the right way round (there is a one-tick cure if not), and whether everything still gets drawn at steep angles. Still waiting on a wearer: does the smear ever show in one eye only? | 09-19 | [re-village-scope-vr](https://github.com/TefMeister/re-village-scope-vr) |

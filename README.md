...
| **RE Village — VR scope** | RE Engine | 🎮 A good night. The picture inside the scope had gone completely black on this machine, and the reason turned out to be a nice one: the mod was grabbing its picture from the wrong place a split second after the game started, minutes before the rifle it belongs to even exists. The giveaway was eight pixels — the real one is always a fraction taller. Caught it, proved the fix in the game, and the view through the scope came straight back. Two more things fell out of it: a measurement that has never actually measured anything since it was written, and the upside-down picture finally has a right way up. Also settled: hiding the player has a working trigger, and the sniper's spread is not held on the gun at all, so that idea needs another route. | 09-18 | [re-village-scope-vr](https://github.com/TefMeister/re-village-scope-vr) |

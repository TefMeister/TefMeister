...
- 🎮 **Hard Reset: first launch, with TefMeister at the keyboard.** The developer console works
  (Ctrl + ~), and the old 3D switch turned out to do *something* on a modern card — which this page
  had just predicted it would not. What it does looks like a mix-up of the game's images rather than
  two views, so the prediction is half wrong and the good news is not in yet. A test of whether the
  game will use an edited shader file is set up and waiting for the next start.
...
| **Hard Reset** (2011) | Road Hog Engine (Direct3D 9) | ⭐ **The game ships its shaders as readable source** (2026-09-14), naming its own camera matrices, and compiles them itself. Its built-in "stereo" settings turned out, on a closer read, to be NVIDIA's retired 3D Vision rather than the game's own — so no free VR renderer, but a very open door. First launch done: the developer console works | [hard-reset-vr](https://github.com/TefMeister/hard-reset-vr) |

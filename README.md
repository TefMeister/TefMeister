...
| **RE Village — VR scope** | RE Engine | 🎮 Working, zeroed sniper scope. A good day with the game switched off: the settings file was quietly saving one-off experiments as permanent choices and no longer does, the tuning switches became buttons reachable inside the headset, the game's log now survives a restart, and the upside-down picture in one scope mode was traced to a single setting being used twice over — which is also why the button meant to fix it did nothing. All installed and waiting on the next start. | 09-18 | [re-village-scope-vr](https://github.com/TefMeister/re-village-scope-vr) |

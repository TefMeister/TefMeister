...
| **RE Village — VR scope** | RE Engine | 🎮 Now chasing the rifle's accuracy: it turns out the gun has its own built-in switches for steadying the aim and softening the recoil, sitting right next to the code that fires the shot. A small tool to test them is written and in place, and one play session should say whether holding the aim button was simply flipping those switches all along — if so, leaving them on is the whole fix. The wearer also re-described the picture fault as a stepping jitter that only appears when the head turns, which points somewhere quite different from where we were looking. | 09-20 | [re-village-scope-vr](https://github.com/TefMeister/re-village-scope-vr) |

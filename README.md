...
| **RE Village — VR scope** | RE Engine | 🎮 The rifle shoots exactly where it points, from the hip, no aim button needed. Today's headset test found the cause of the two-handed miss: the gun jumps a little just before the bullet leaves, and the bullet followed the jump. The fix, aiming the bullet where the crosshair was a moment earlier, is built and goes in next. The mod's new distance measurement also works, which should cure the small one-handed offset at close range. | 09-21 | [re-village-scope-vr](https://github.com/TefMeister/re-village-scope-vr) |

...
| **RE Village — VR scope** | RE Engine | 🎮 The fix for the smearing scope picture is installed, switched on, and **confirmed running in the real game** — it rewrote the scope picture 6,600 times in a minute, reading exactly the numbers predicted on paper. Rather than drawing a wider picture and losing sharpness, the scope aims its frame at the barrel, so the part it crops out always sits in the middle and cannot run off the edge. One thing still to see: whether the picture really follows when the angle changes, and whether the game keeps drawing everything at steep angles. A one-click undo is in place and tested both ways. | 09-19 | [re-village-scope-vr](https://github.com/TefMeister/re-village-scope-vr) |

...
| **RE Village — VR scope** | RE Engine | 🎮 Working, zeroed sniper scope. Two fronts moved this week without the game running: the player's own clothing cannot be masked out of the scope alone, so the body is now hidden for exactly as long as the scope is raised; and the flicker measurement that kept reporting nothing now says which of two causes it is. Both built and installed, both waiting on one start of the game. | 09-18 | [re-village-scope-vr](https://github.com/TefMeister/re-village-scope-vr) |
...
| **The Darkness** (2007) | Starbreeze engine | ⭐ **Plays in 3D; the exact spot to give each eye its own camera is found, and how to attach the test is now settled.** A hidden developer menu and free camera turned out to be in the retail game, but locked. Next: trying the stereo test live | 09-18 | [the-darkness-vr](https://github.com/TefMeister/the-darkness-vr) |

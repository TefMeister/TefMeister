...
| **RE Village — VR scope** | RE Engine | 🎮 The rifle shoots exactly where it points, from the hip, no aim button needed. What is left is a small, steady miss to the left when holding the rifle with both hands. Found why it is likely: the scope's aim correction tilts along with the rifle, so a two-handed grip that tilts the gun shifts the crosshair by several degrees. Every shot now records the tilt, so one short test in the headset will settle it. | 09-21 | [re-village-scope-vr](https://github.com/TefMeister/re-village-scope-vr) |

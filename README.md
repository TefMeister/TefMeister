...
| **RE Village — VR scope** | RE Engine | 🎮 Two-handed shots now land on the crosshair, confirmed in the headset. The distance correction that was never reaching the scope picture is fixed and seen working in the headset, the scope was re-zeroed with it on, and shots at a near and a far target then all landed on the crosshair without touching the zero — and did so again after restarting the game, ten shots out of ten. A second day will settle it. The 1080p scope picture now switches on and looks no softer. Newest: the rifle being thrown to the left when the second hand goes on was traced to how the grip starts, and a fix is built and waiting for a headset run. | 09-21 | [re-village-scope-vr](https://github.com/TefMeister/re-village-scope-vr) |

...
| **RE Village — VR scope** | RE Engine | 🔍 Spent the evening building a fix for the smearing scope picture, tested it in the game, and **proved it cannot work** — the part of the game we were changing turns out not to be the part that draws the scope picture at all. A deliberately unmissable test settled it in one tick. That also undoes the theory the fix was built on, which had looked solid and had twenty passing checks behind it; the sums were right, they were just about the wrong thing. Honest dead end, cheaply reached, and it leaves a much better question: find what really draws that picture. That part needs no game running. | 09-19 | [re-village-scope-vr](https://github.com/TefMeister/re-village-scope-vr) |

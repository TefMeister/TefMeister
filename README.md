...
| **RE Village — VR scope** | RE Engine | 🎮 Working, zeroed sniper scope. Latest: the player's own clothing getting in the way of the scope picture. It cannot be masked out of the scope alone — the engine has no such switch — so instead the body is now hidden for exactly as long as the scope is raised, using the game's own "scope is up" flag. Written and installed, not yet run. | 09-18 | [re-village-scope-vr](https://github.com/TefMeister/re-village-scope-vr) |
